...
class AutoInt(BaseModel):
    ...
    def forward(self, X):
        ...
        sparse_input_flatten = torch.flatten(sparse_input, start_dim=1)
        if len(dense_value_list)>0:
            dense_input = torch.cat(dense_value_list, dim=1)
            dnn_input = concat_fun([sparse_input_flatten, dense_input])
        else:
            dnn_input = sparse_input_flatten
